...
vector<int> getPostorder(const vector<int>& preorder, const vector<int>& inorder) {
    ...
    vector<int> left = getPostorder(vector<int>(preorder.begin() + 1, preorder.begin() + 1 + rootPos), vector<int>(inorder.begin(), inorder.begin() + rootPos));
    vector<int> right = getPostorder(vector<int>(preorder.begin() + 1 + rootPos, preorder.end()), vector<int>(inorder.begin() + rootPos + 1, inorder.end()));
 
    // Concatenate the left, right, and root traversals to get the postorder traversal
    vector<int> result;
