...
void conv2d_openacc(const float* input, const float* filters, float* output, 
    ...
    #pragma acc data present(input, filters, output)
    {
...
void max_pooling_openacc(const float* input, float* output, size_t batch_size, 
    ...
    #pragma acc data present(input, output)
    {
...
void init_openacc(float* conv_weights, size_t conv_weights_size, float* fc_weights, size_t fc_weights_size){
...
void compute_obj_grad_cnn_openacc(const DataSet *data, float *W1, float *W2,
  ...
  #pragma acc enter data create(tmp[0:data->images_num * hidden_dim])
    ...
    matrix_dot_openacc(data->images_matrix, W1, tmp, data->images_num, data->input_dim,
                hidden_dim);
    ReLU_cnn_openacc(tmp, data->images_num * hidden_dim);
    matrix_dot_openacc(tmp, W2, result, data->images_num, hidden_dim, num_classes);
    }

    // #pragma exit data delete(tmp)
  delete[] tmp;
...
void backprop_cnn_openacc(const float *X, const unsigned char *y, const float *output, 
    ...
    #pragma acc data present(X, y, output)
    #pragma acc data create(grad_FCWeights)
    {
...
void cnn_epoch_openacc(const float *X, const unsigned char *y, float *ConvFilters, 
    ...
    #pragma acc data present(X, y, ConvFilters, FCWeights)
    {
    ...
    #pragma acc data create(conv_output, pool_output, flattened_output, output)
    for (size_t i = 0; i < m; i += batch) {
        size_t current_batch_size = std::min(batch, m - i);

        // Forward pass
        conv2d_openacc(X + i * n, ConvFilters, conv_output, current_batch_size, n, num_filters, filter_size);
        ReLU_cnn_openacc(conv_output, current_batch_size * conv_output_dim);
        max_pooling_openacc(conv_output, pool_output, current_batch_size, conv_output_dim, pool_size);
        flatten_openacc(pool_output, flattened_output, current_batch_size, pool_output_dim);
        matrix_dot_openacc(flattened_output, FCWeights, output, current_batch_size, flattened_output_dim, num_classes);
        matrix_softmax_normalize_openacc(output, current_batch_size, num_classes);

        // Backward pass
        backprop_cnn_openacc(X + i * n, y + i, output, ConvFilters, FCWeights, current_batch_size, n, num_filters, filter_size, num_classes, batch, lr);
    }
...
void ReLU_openacc_cnn(float *M, size_t size) {
  #pragma acc parallel loop independent
  for (size_t i = 0; i < size; ++i) {
    if (M[i] < 0)
      M[i] = 0;
...
void compute_obj_grad_openacc_cnn(const DataSet *data, float *W1, float *W2,
                      size_t num_classes, size_t hidden_dim, float *result) {
  float *tmp = new float[data->images_num * hidden_dim];
  ...
  #pragma acc enter data create(tmp[0:data->images_num * hidden_dim])
  {
    matrix_dot_openacc(data->images_matrix, W1, tmp, data->images_num, data->input_dim,
              hidden_dim);
    ReLU_openacc_cnn(tmp, data->images_num * hidden_dim);
    matrix_dot_openacc(tmp, W2, result, data->images_num, hidden_dim, num_classes);
  }
...
void train_cnn_openacc(const DataSet *train_data, const DataSet *test_data,
    ...
    std::mt19937 rng;
    rng.seed(0);
    std::normal_distribution<float> dist(0.0, std::sqrt(1.0 / (num_filters * filter_size * filter_size / pool_size)));
    for (size_t i = 0; i < conv_weights_size; i++) {
    ...
    init_openacc(conv_weights, conv_weights_size, fc_weights, fc_weights_size);
  ...
  auto start_time = std::chrono::high_resolution_clock::now();
    #pragma acc enter data copyin(  train_data -> images_matrix[0:train_data->images_num * train_data->input_dim],\
                                  ...
                                  conv_weights[0:conv_weights_size], fc_weights[0:fc_weights_size])
    #pragma acc enter data create(train_result[0:train_data->images_num * num_classes], test_result[0:test_data->images_num * num_classes])
    for (size_t epoch = 0; epoch < 2 * epochs; epoch++) {
        float current_lr = initial_lr / (1 + epoch * 0.1);
        nn_epoch_openacc(train_data->images_matrix, train_data->labels_array, conv_weights, fc_weights,
                 train_data->images_num, train_data->input_dim, hidden_dim,
                 num_classes, lr, batch);

        if(epoch % 2 == 1) continue;
                 
        compute_obj_grad_openacc_cnn(train_data, conv_weights, fc_weights, num_classes, hidden_dim, train_result);
        compute_obj_grad_openacc_cnn(test_data, conv_weights, fc_weights, num_classes, hidden_dim, test_result);

        train_loss = mean_softmax_loss_openacc(train_result, train_data->labels_array,
